...
# Authentication: ${method.authentication}
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._check_job",
        params => \@args});
    if ($result) {
        if ($result->is_error) {
...
{
    my($self, @args) = @_;
    my $job_id = $self->_TranslateNucToProtSeq_submit(@args);
...
sub _TranslateNucToProtSeq_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _TranslateNucToProtSeq_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        ...
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _TranslateNucToProtSeq_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_TranslateNucToProtSeq_submit');
        }
    }
    my $context = undef;
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._TranslateNucToProtSeq_submit",
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _TranslateNucToProtSeq_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_TranslateNucToProtSeq_submit');
    }
}
...
{
    my($self, @args) = @_;
    my $job_id = $self->_ParseFastaStr_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _ParseFastaStr_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _ParseFastaStr_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _ParseFastaStr_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_ParseFastaStr_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._ParseFastaStr_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _ParseFastaStr_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_ParseFastaStr_submit');
    }
}
...
{
    my($self, @args) = @_;
    my $job_id = $self->_GenomeToFASTA_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _GenomeToFASTA_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _GenomeToFASTA_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _GenomeToFASTA_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_GenomeToFASTA_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._GenomeToFASTA_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _GenomeToFASTA_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_GenomeToFASTA_submit');
    }
}
...
{
    my($self, @args) = @_;
    my $job_id = $self->_GenomeSetToFASTA_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _GenomeSetToFASTA_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _GenomeSetToFASTA_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _GenomeSetToFASTA_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_GenomeSetToFASTA_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._GenomeSetToFASTA_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _GenomeSetToFASTA_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_GenomeSetToFASTA_submit');
    }
}
...
{
    my($self, @args) = @_;
    my $job_id = $self->_FeatureSetToFASTA_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _FeatureSetToFASTA_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _FeatureSetToFASTA_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _FeatureSetToFASTA_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_FeatureSetToFASTA_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._FeatureSetToFASTA_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _FeatureSetToFASTA_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_FeatureSetToFASTA_submit');
    }
}
...
sub status
{
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "KBaseDataObjectToFileUtils._status_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
