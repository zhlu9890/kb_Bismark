package SetAPI::SetAPIServiceClient;
...
sub new
{
    ...
    my $self = {
	client => SetAPI::SetAPIServiceClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: optional
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function get_differential_expression_matrix_set_v1 (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        ...
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to get_differential_expression_matrix_set_v1:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'get_differential_expression_matrix_set_v1');
	}
    }

    my $service_state = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "ServiceWizard.get_service_status",
        params => [{module_name=>"SetAPI", version=>$self->{service_version}}]});
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "SetAPI.get_differential_expression_matrix_set_v1",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method get_differential_expression_matrix_set_v1",
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function save_differential_expression_matrix_set_v1 (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to save_differential_expression_matrix_set_v1:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'save_differential_expression_matrix_set_v1');
	}
    }
    ...
    my $url = $service_state->result->[0]->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "SetAPI.save_differential_expression_matrix_set_v1",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method save_differential_expression_matrix_set_v1",
					    status_line => $self->{client}->status_line,
					    method_name => 'save_differential_expression_matrix_set_v1',
				       );
    }
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function create_sample_set (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to create_sample_set:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'create_sample_set');
	}
    }
    ...
    my $url = $service_state->result->[0]->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "SetAPI.create_sample_set",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method create_sample_set",
					    status_line => $self->{client}->status_line,
					    method_name => 'create_sample_set',
				       );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for SetAPI::SetAPIServiceClient\n";
    }
    if ($sMajor == 0) {
        warn "SetAPI::SetAPIServiceClient version is $svr_version. API subject to change.\n";
    }
}
...
package SetAPI::SetAPIServiceClient::RpcClient;
