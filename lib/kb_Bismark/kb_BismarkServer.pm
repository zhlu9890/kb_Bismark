package kb_Bismark::kb_BismarkServer;
...
our %return_counts = (
        'genome_preparation' => 1,
        'bismark' => 1,
        'methylation_extractor' => 1,
        'bismark_app' => 1,
        'run_bismark_cli' => 0,
        'status' => 1,
);
...
our %method_authentication = (
        'genome_preparation' => 'required',
        'bismark' => 'required',
        'methylation_extractor' => 'required',
        'bismark_app' => 'required',
        'run_bismark_cli' => 'required',
);
...
sub call_method {
    ...
    my $ctx = kb_Bismark::kb_BismarkServerContext->new($self->{loggers}->{userlog},
...
{
    # Service kb_Bismark requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for kb_Bismark but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = kb_Bismark::kb_BismarkServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service kb_Bismark.");
    }
...
package kb_Bismark::kb_BismarkServerContext;

use strict;

=head1 NAME

kb_Bismark::kb_BismarkServerContext
...
package kb_Bismark::kb_BismarkServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = kb_Bismark::kb_BismarkServer->new(
