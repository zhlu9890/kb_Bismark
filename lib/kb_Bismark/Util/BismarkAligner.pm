...
sub single_reads_lib_run {
  ...
  # run methylationExtractor
  my $extractor_output_info = $self->run_bismark_methylation_extractor($run_output_info, $input_configuration, $validated_params); 
  
  my $report_info;
  if ($create_report) {
    $report_info = $self->create_report_for_single_run($run_output_info, $input_configuration, $extractor_output_info, $validated_params);
  }
...
sub run_bismark_methylation_extractor {
  ...
  my $extractor = kb_Bismark::Util::BismarkMethylationExtractor->new(
    $self->{scratch},
    $self->{workspace_url},
    $self->{callback_url},
    $self->{srv_wiz_url},
  ...
  my $extractor_output_info = $extractor->extract($run_output_info, $input_configuration, $validated_params);
}
...
sub create_report_for_single_run {
  my ($self, $run_output_info, $input_configuration, $extractor_output_info, $validated_params) = @_;
  ...
  my $output_dir=$run_output_info->{output_dir};
  my $html_dir=File::Spec->catfile($output_dir, 'html');
  mkdir $html_dir;
  system("cd $output_dir && bismark2report && mv $output_dir/*_report.html $html_dir");
  my ($source_name)=read_dir($html_dir);

  my $html_string=read_file('/kb/module/data/index_start.txt');
  $html_string.='<iframe id="content" style="width: 100%; border: none;" src="' . $source_name . '"></iframe>';
  $html_string.=read_file('/kb/module/data/index_end.txt');

  write_file("$html_dir/index.html", $html_string);
  ...
  my $output_html_files=[{
      ...
      name => 'index.html',
      label => 'html files',
      description => 'Report HTML files generated by Bismark',
    }
  ];

  my $output_files=$extractor_output_info->{output_file};
